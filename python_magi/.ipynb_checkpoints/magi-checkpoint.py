...
class MAGI:
    ...
    def __init__(self, D_thetas : int, 
                 ts_obs: np.ndarray, 
                 X_obs : np.ndarray, 
                 bandsize : Union[int, None], 
                 f_vec: Callable) -> None:
        '''Constructor for MAGI class.
        
        Args:
            D_thetas (int): number of parameters governing our system.
            ts_obs (np.ndarray): timesteps in our data
            X_obs (np.ndarray): matrix of observed + missing values in our data.
            bandsize (int/None): are we doing band-matrix approximations? If None, then no. Else, positive integer bandsize.
            f_vec (Callable): batchable function. Given timesteps t, observations X, & parameters theta, return dX/dt at said timesteps.
        '''
    ...
    def initial_fit(self, discretization : int, 
                    ...
                    use_fourier_prior : bool = True,
                    phi1s_exog : Union[np.ndarray, None] = None, 
                    phi2s_exog : Union[np.ndarray, None] = None, 
                    sigma_sqs_exog : Union[np.ndarray, None] = None,
                    no_noise_on_inferred : bool = True,
                    verbose : bool = False) -> None:
        '''Fits initial Matern kernel hyperparameters for inverse problem.
        
        Args:
            discretization (int): inserting 2^d - 1 timesteps in between consecutive observations.
            fit_phi_with_interpolation (bool): do we use linearly-interpolated observations for fitting phi, or original (ts_obs, X_obs)?
            use_fourier_prior (bool): is system appropriate for fourier analysis? else use flat prior.
            phi1s_exog (np.array/None): user pre-specified values for Matern kernel phi1 hyperparameters. If None, will be fitted.
            phi2s_exog (np.array/None): user pre-specified values for Matern kernel phi2 hyperparameters. If None, will be fitted.
            sigma_sqs_exog (np.array/None): user pre-specified values for Gaussian process component noise parameters. If None, will be fitted.
            no_noise_on_inferred (bool): if a component is unobserved/only based on inferred, should sigma_sq be set to basically zero?
            verbose (bool): progress bars or no?
        '''
    ...
    def log_joint_given_theta(self,
                              X: tf.Tensor,          # shape (m , D)
                              sigma_sqs: tf.Tensor,  # shape (D,)
                              theta_vec: tf.Tensor   # shape (P,)
                              ) -> tf.Tensor:
        """
        Calculates the log-likelihood based on Eq. [5] from Yang, Wong & Kou (PNAS 2020),
        excluding the prior on theta. This version includes the log-determinant terms
        for a complete likelihood calculation.

        Parameters
        ----------
        X         : latent trajectory on the dense grid I
        sigma_sqs : observation-noise variances (positive)
        theta_vec : ODE parameters
        
        Returns
        -------
        log_like  : scalar tf.Tensor = log p(X, y | θ, σ², ψ)
        """
        # ---------- Conveniences ----------
        mu_ds   = self.mu_ds           # (D,)
        Cinv    = self.C_d_invs         # (D , m , m)
        m_ds    = self.m_ds           # (D , m , m)
        Kinv    = self.K_d_invs         # (D , m , m)
        Igrid   = self.I              # (m , 1)
        f_vec   = self.f_vec          # user ODE rhs
        mag_I   = tf.cast(tf.shape(X)[0], tf.float64)

        # Observation index helpers
        idxs    = self.not_nan_idxs       # flattened indices of observed entries
        cols    = self.not_nan_cols       # which component each idx belongs to
        y_obs   = self.y_tau_ds_observed
        N_ds    = self.N_ds              # number of obs per component D

        # ========== Term (1): GP Prior on X(I) ==========
        # Centered trajectory: X − μ → shape (m, 1, D)
        X_cent = tf.reshape(X - mu_ds, (tf.shape(X)[0], 1, tf.shape(X)[1]))
        
        # Quadratic part: ||x_d(I) - μ_d(I)||^2_{C_d^-1}
        mahalanobis_C = tf.reduce_sum(
            (tf.transpose(X_cent) @ Cinv) @ tf.transpose(X_cent, [2, 0, 1]))
        
        # Log-determinant part: log|C_d| = -log|C_d^-1|
        log_det_C = -tf.linalg.logdet(Cinv) # shape (D,)

        # Full Term (1) from equation, summed over all D components
        Term1 = tf.reduce_sum(mag_I *tf.math.log(tf.constant(2.0 * np.pi, dtype=tf.float64)) + log_det_C) + mahalanobis_C

        # ========== Term (3): ODE Mismatch Term ==========
        # Evaluate f(I, X, θ) ; reshape to (D, m, 1) for broadcasting
        f_vals = tf.reshape(f_vec(Igrid, X, theta_vec),
                            (tf.shape(X)[1], tf.shape(X)[0], 1))
        
        # Residual: f − μ̇ − m(X−μ)
        resid = f_vals - (m_ds @ tf.transpose(X_cent, [2, 0, 1]))
        
        # Quadratic part: ||f - E[f|X]||^2_{K_d^-1}
        mahalanobis_K = tf.reduce_sum(
            tf.transpose(resid, [0, 2, 1]) @ (Kinv @ resid))
            
        # Log-determinant part: log|K_d| = -log|K_d^-1|
        log_det_K = -tf.linalg.logdet(Kinv) # shape (D,)

        # Full Term (3) from equation, summed over all D components
        Term3 = tf.reduce_sum(mag_I * tf.math.log(tf.constant(2.0 * np.pi, dtype=tf.float64)) + log_det_K) + mahalanobis_K
        
        # ========== Term (2): Data Likelihood Term ==========
        # This term is already correctly implemented as Q3 in the original code
        
        # Log-determinant part: N_d * log(2πσ_d^2)
        log_det_Sigma = tf.reduce_sum(
            N_ds * tf.math.log(tf.constant(2.0 * np.pi, dtype=tf.float64) * sigma_sqs))
            
        # Quadratic part: ||x_d(τ_d) - y_d(τ_d)||^2_{σ_d^-2}
        X_obs = tf.gather(tf.reshape(X, [-1]), idxs)
        mahalanobis_Sigma = tf.reduce_sum(
            tf.square(X_obs - y_obs) * tf.gather(tf.constant(1.0, dtype=tf.float64) / sigma_sqs, cols))
            
        Term2 = log_det_Sigma + mahalanobis_Sigma

        # ========== Final Log-Likelihood ==========
        # As per the equation: exp{ -1/2 * sum(...) }
        return -0.5 * (Term1 + Term3 + Term2)


    # ================================================================
    #  optimise_theta_window  –  Adam  (joint X & θ)
    # ================================================================
    def optimise_theta_window(self,
                              start:       int   = 0,
                              stop:        int | None = None,
                              lr_theta:    float = 5e-3,
                              lr_X:        float = 1e-2,
                              ...
                              patience:    int   = 20):
        """
        Joint MAP of (X_window , θ) with two Adam optimisers.
    
        Early-stop when best NLL improves by < tol for `patience`
        consecutive *print intervals*.
    
        Returns
        -------
        { "theta_hat", "X_hat", "loglik", "pr" }
        """
        # ---------- slice the dense grid ----------
        if stop is None:
            stop = self.mag_I
        X0_slice  = self.Xhat_init[start:stop]              # (m , D)
        sigma_tf  = tf.convert_to_tensor(self.sigma_sqs_init, tf.float64)
    
        # ---------- trainable variables ----------
        X_var     = tf.Variable(X0_slice,  dtype=tf.float64, name="X_latent")
        theta_var = tf.Variable(self.thetas_init, dtype=tf.float64, name="theta")
    
        opt_theta = tf.keras.optimizers.Adam(lr_theta)
        opt_X     = tf.keras.optimizers.Adam(lr_X)
    
        @tf.function
        def train_step():
            with tf.GradientTape(persistent=True) as tape:
                nll = -self.log_joint_given_theta(X_var, sigma_tf, theta_var)
            # gradients
            g_theta = tape.gradient(nll, theta_var)
            g_X     = tape.gradient(nll, X_var)
            # apply
            opt_theta.apply_gradients([(g_theta, theta_var)])
            opt_X.apply_gradients([(g_X,     X_var    )])
            return nll
    
        # ---------- early-stopping bookkeeping ----------
        best_nll   = np.inf
        best_theta = theta_var.numpy().copy()
        best_X     = X_var.numpy().copy()
        stall_ctr  = 0
        ...
        for i in range(1, iters + 1):
            ...
            # progress prints + early-stop
            if (i % print_every == 0) or (i == iters):
                if verbose:
                    print(f"[Adam-joint] iter {i:5d}  log-lik = {-nll.numpy():.6g}")
            stall_ctr = 0 if improved else stall_ctr + 1
            if stall_ctr >= patience:
                if verbose:
                    print(f"[Adam-joint] early stop @ iter {i}")
                break
    
        # ---------- pack results ----------
        loglik = -best_nll.numpy()
        pr     = float(np.exp(loglik))
        self.thetas_init = best_theta.copy()   # warm-start next call
        self.Xhat_init[start:stop] = best_X    # keep best path
    
        return {"theta_hat": best_theta,
                "X_hat":     best_X,
                "loglik":    loglik,
                "pr":        pr}
    ...
    def predict(self,    
                ...
                sigma_sqs_LB : Union[np.ndarray, None] = None, 
                ...
                map_iters: int = 5000,
                map_lr_X: float = 1e-4,
                #map_lr_sigma: float = 5e-3,
                map_lr_theta: float = 1e-4,
                map_print_every: int = 200,
                map_tol: float = 1e-12,
                map_patience: int = 20,
                map_beta_temp: float = 1.0,
             ) -> dict:
        '''Returns posterior samples and diagnostics of inverse problem system.
        
        Args:
            num_results (int): number of MCMC samples to take from our posterior distribution.
            num_burnin_steps (int): number of MCMC steps to take before collecting samples for downstream analysis.
            sigma_sqs_LB (np.array/None): lower bounds for sampled component-wise noise parameters.
            tempering (bool): are we tempering thet log-posterior to encourage more initial exploration? (EXPERIMENTAL)
            verbose (bool): progress bars or no?

        Returns:
            results (dict): fitted Matern kernel hyperparameters, posterior samples, and diagnostics.
        '''
        ...
        def unnormalized_log_prob_no_temp(X, sigma_sqs_pre, thetas_pre):
            return unnormalized_log_prob(X, sigma_sqs_pre, thetas_pre, beta_temp=tf.constant(1.0, dtype=tf.float64))
        ...
        def logpost_constrained_no_jac(X, sigma_sqs, thetas, beta_temp):
            """
            Log-posterior in the original parameterization; no change-of-variables terms.
            Assumes sigma_sqs >= sigma_sqs_LB and thetas > 0 are already enforced.
            """
            beta_temp = tf.stop_gradient(beta_temp)
            ...
            # t1: ||x_D(I) - μ_d(I)||_{C_d^-1}^2
            t1 = tf.reduce_sum((tf.transpose(X_cent) @ C_d_invs) @ tf.transpose(X_cent, perm=[2, 0, 1]))
        
            # t2: ||f_d(I) - μ̇_d(I) - m_d{ x_d(I) - μ_d(I) }||_{K_d^-1}^2
            f_vals = tf.transpose(f_vec(I, X, thetas)[:, None], perm=[2, 0, 1])
            toNorm = f_vals - (m_ds @ tf.transpose(X_cent, perm=[2, 0, 1]))
            t2 = tf.reduce_sum(tf.transpose(toNorm, perm=[0, 2, 1]) @ (K_d_invs @ toNorm))
            ...
            # t4: ||x_d(τ_d) - y_d(τ_d)||_{σ_d^{-2}}^2  (only observed entries)
            X_observed = tf.gather(tf.reshape(X, [-1]), not_nan_idxs)
            t4 = tf.reduce_sum(
                tf.square(X_observed - y_tau_ds_observed) *
                tf.gather(1.0 / sigma_sqs, not_nan_cols)
            )
            
            return beta_temp * (-0.5 * (((t1 + t2)) + (t3 + t4)))

        
        # =========================
        # MAP branch (no sampling) — optimize X and θ ONLY; σ² fixed at init
        # =========================
        '''if method.lower() == "map":
            # --- Random init for X and thetas ---
            rng = np.random.default_rng(seed=42)   # or use self.SEED if you stored one
        
            X_init_rand       = 5 * rng.standard_normal(self.Xhat_init.shape)
            thetas_init_rand  = 5 * rng.standard_normal(self.D_thetas)
        
            # Variables to optimize
            X_state      = tf.Variable(X_init_rand,      dtype=tf.float64, name="X_state")
            thetas_state = tf.Variable(thetas_init_rand, dtype=tf.float64, name="thetas_state")
        
            # σ² is kept FIXED (constant tensor, not trainable)
            sigma_const = tf.convert_to_tensor(self.sigma_sqs_init, dtype=tf.float64)
        
            # Opts only for X and θ
            opt_X     = tf.keras.optimizers.Adam(map_lr_X)
            opt_theta = tf.keras.optimizers.Adam(map_lr_theta)
            ...
            def step():
                with tf.GradientTape(persistent=True) as tape:
                    # NEGATIVE log-posterior (Eq.5, no Jacobians)
                    nlp = -logpost_constrained_no_jac(
                        X=X_state,
                        sigma_sqs=sigma_const,
                        thetas=thetas_state,
                        beta_temp=tf.constant(1.0, dtype=tf.float64)
                    )
                gX, gt = tape.gradient(nlp, [X_state, thetas_state])
                opt_X.apply_gradients([(gX, X_state)])
                opt_theta.apply_gradients([(gt, thetas_state)])
                return nlp
            best_nlp  = np.inf
            stall     = 0
            best_pack = (self.Xhat_init.copy(),
                         self.sigma_sqs_init.copy(),         # unchanged
                         self.thetas_init.copy())
        
            for i in range(1, map_iters + 1):
                nlp = step()
                nlp_val = float(nlp.numpy())
        
                if nlp_val < best_nlp - map_tol:
                    best_nlp = nlp_val
                    best_pack = (X_state.numpy().copy(),
                                 self.sigma_sqs_init.copy(),          # still fixed
                                 thetas_state.numpy().copy())
                    stall = 0
                else:
                    stall += 1
        
                if (i % map_print_every == 0 or i == map_iters) and verbose:
                    print(f"[MAP] iter {i:5d}  neg-log-post={nlp_val:.6g}")
        
                if stall >= map_patience:
                    if verbose:
                        print(f"[MAP] early stop @ iter {i}")
                    break
        
            X_map, sigma_map, theta_map = best_pack
        
            # Warm start future calls (σ² unchanged)
            self.Xhat_init      = X_map.copy()
            self.sigma_sqs_init = sigma_map.copy()
            self.thetas_init    = theta_map.copy()
        
            return {
                "phi1s": self.phi1s, "phi2s": self.phi2s,
                "I": self.I,
                "X_map": X_map,
                "sigma_sqs_map": sigma_map,   # identical to init
                "theta_hat": theta_map,
                "loglik": -best_nlp,
                "minutes_elapsed": 0.0,
                "method": "map"
        ...
        # =========================
        # MAP branch (X and θ only; σ² fixed at init)
        # =========================
        if method.lower() == "map":
            if optimizer.lower() == "lbfgs":
                rng = np.random.default_rng(seed=42)
        
                # You can switch these to your interpolants if preferred
                X_init_rand       = 0.5 * rng.standard_normal(self.Xhat_init.shape)
                thetas_init_rand  = 0.5 * rng.standard_normal(self.D_thetas)
        
                # σ² is FIXED during MAP
                sigma_const = tf.convert_to_tensor(self.sigma_sqs_init, dtype=tf.float64)
        
                # Helpers to pack/unpack (X, θ) ↔ 1D vector for L-BFGS
                X_shape     = tuple(X_init_rand.shape)
                theta_shape = (self.D_thetas,)
                X_size      = int(np.prod(X_shape))
                theta_size  = int(np.prod(theta_shape))
        
                def pack_vars(X_tf, theta_tf):
                    X_flat     = tf.reshape(X_tf,     [-1])
                    theta_flat = tf.reshape(theta_tf, [-1])
                    return tf.concat([X_flat, theta_flat], axis=0)
        
                def unpack_vars(w_flat):
                    X_flat     = w_flat[:X_size]
                    theta_flat = w_flat[X_size:X_size + theta_size]
                    X_tf       = tf.reshape(X_flat,     X_shape)
                    theta_tf   = tf.reshape(theta_flat, theta_shape)
                    return X_tf, theta_tf
        
                # Initial position
                w0 = tf.convert_to_tensor(
                    np.concatenate([X_init_rand.reshape(-1), thetas_init_rand.reshape(-1)]),
                    dtype=tf.float64
                )
        
                @tf.function(jit_compile=False)
                def value_and_gradients_fn(w_flat):
                    with tf.GradientTape() as tape:
                        tape.watch(w_flat)
                        X_tf, theta_tf = unpack_vars(w_flat)
                        # NEGATIVE log-posterior (Eq.5, no Jacobians; σ² fixed)
                        nlp = -logpost_constrained_no_jac(
                            X=X_tf,
                            sigma_sqs=sigma_const,
                            thetas=theta_tf,
                ...
                results = tfp.optimizer.lbfgs_minimize(
                    ...
                    initial_position=w0,
                    max_iterations=int(map_iters),
                    tolerance=float(map_tol),
                    parallel_iterations=1,
                    num_correction_pairs=10,
                )
        
                # Unpack solution
                X_opt_tf, theta_opt_tf = unpack_vars(results.position)
                X_map      = X_opt_tf.numpy().copy()
                theta_map  = theta_opt_tf.numpy().copy()
                best_nlp   = float(results.objective_value.numpy())
        
                # Optional diagnostics
                if verbose:
                    try:
                        grad_norm = float(tf.linalg.norm(results.objective_gradient).numpy())
                    except Exception:
                        grad_norm = float("nan")
                    conv = "converged" if bool(results.converged.numpy()) else "NOT converged"
                    print(f"[MAP/L-BFGS] {conv} in {int(results.num_iterations.numpy())} iters  "
                          f"neg-log-post={best_nlp:.6g}  grad_norm={grad_norm:.3e}")
        
                return {
                    "phi1s": self.phi1s, "phi2s": self.phi2s,
                    "I": self.I,
                    "X_hat": X_map,
                    "sigma_sqs_map": sigma_const,   # unchanged (fixed)
                    "theta_hat": theta_map,
                    "loglik": -best_nlp,
                    "minutes_elapsed": 0.0,
                    "method": "map_lbfgs",
                ...
                # Use your existing warm-starts (recommended)
                rng = np.random.default_rng(seed=42)
                # X_init_rand      = self.Xhat_init
                # thetas_init_rand = self.thetas_init
                # X_init_rand       = 0.5 * rng.standard_normal(self.Xhat_init.shape)
                # thetas_init_rand  = 0.5 * rng.standard_normal(self.D_thetas)
                X_init_rand = tf.convert_to_tensor(X_init, dtype=tf.float64)
                thetas_init_rand = tf.convert_to_tensor(theta_init, dtype=tf.float64)
                sigma_const = tf.convert_to_tensor(self.sigma_sqs_init, dtype=tf.float64)
        
                # Trainables
                X_var     = tf.Variable(X_init_rand,      dtype=tf.float64, name="X_state")
                theta_var = tf.Variable(thetas_init_rand, dtype=tf.float64, name="theta_state")
                ...
                def step():
                    ...
                    grads = tape.gradient(nlp, [X_var, theta_var])
                    opt.apply_gradients(zip(grads, [X_var, theta_var]))
                    gnorm = tf.linalg.global_norm([g for g in grads if g is not None])
                    return nlp, gnorm
        
                for it in range(1, int(map_iters) + 1):
                    nlp, gnorm = step()
                    nlp_val = float(nlp.numpy())
        
                    # "same as commented way": absolute improvement with tolerance + patience
                    if nlp_val < best_nlp - float(map_tol):
                        best_nlp   = nlp_val
                        best_X     = X_var.numpy().copy()
                        best_theta = theta_var.numpy().copy()
                        stall_ctr  = 0
                    ...
                    if (it % max(1, int(map_print_every)) == 0 or it == int(map_iters)) and verbose:
                        try:
                            g_v = float(gnorm.numpy())
                        except Exception:
                            g_v = float("nan")
                        print(f"[MAP/Adam] iter {it:6d}  neg-log-post={nlp_val:.6g}  "
                              f"best={best_nlp:.6g}  stall={stall_ctr}  grad_norm={g_v:.3e}")
        
                    if stall_ctr >= int(map_patience):
                        if verbose:
                            print(f"[MAP/Adam] early stop @ iter {it}")
                        break
        
                # Return best pack we actually saw
                X_map, theta_map = best_X, best_theta
    ...
    #             verbose: bool = False,
    #             # NEW:
    #             method: str = "mcmc",          # "mcmc" or "map"
                # map_iters: int = 5000,
                # map_lr_X: float = 1e-2,
                # map_lr_sigma: float = 5e-3,
                # map_lr_theta: float = 5e-3,
                # map_print_every: int = 200,
                # map_tol: float = 1e-6,
                # map_patience: int = 20,
                # map_beta_temp: float = 1.0
    ...
    #     """
    #     If method="mcmc" (default): run NUTS using a single inner sampler_log_prob().
    #     If method="map": directly maximize the same objective (softplus + Jacobians).
    #     """
    #     # --- guards ---
    #     assert ~np.any(np.isnan(self.Xhat_init)), "Please make sure Xhat_init does not have NaNs."
    #     assert ~np.any(np.isnan(self.sigma_sqs_init)), "Please make sure sigma_sqs_init does not have NaNs."
    ...
    #     if sigma_sqs_LB is None:
    #         sigma_sqs_LB = np.array([0.0] * self.D, dtype=np.float64)
    #     sigma_lb_tf = tf.convert_to_tensor(sigma_sqs_LB, tf.float64)
        
    #     # --- bind locals (avoid Python lookups in tf.function) ---
    #     mu_ds, C_d_invs, f_vec, I = self.mu_ds, self.C_d_invs, self.f_vec, self.I
    #     m_ds, K_d_invs = self.m_ds, self.K_d_invs
    #     N_ds, not_nan_idxs = self.N_ds, self.not_nan_idxs
    #     y_tau_ds_observed, not_nan_cols, beta = self.y_tau_ds_observed, self.not_nan_cols, self.beta
        
    #     # -------- ONE canonical objective used everywhere --------
    #     def sampler_log_prob(X, sigma_sqs_pre, thetas_pre, beta_temp):
    #         """
    #         Exact same target as your previous sampler:
    #         softplus constraints + log-Jacobians + (optional) tempering.
    #         """
    #         # constrain
    #         sigma_sqs = tf.math.softplus(sigma_sqs_pre) + sigma_lb_tf
    #         thetas    = tf.math.softplus(thetas_pre)
        
    #         # jacobians
    #         log_jac_sigma = tf.reduce_sum(tf.math.log_sigmoid(sigma_sqs_pre))
    #         log_jac_theta = tf.reduce_sum(tf.math.log_sigmoid(thetas_pre))
        
    #         # centered trajectory
    #         X_cent_T = tf.transpose(X - mu_ds)  # (D, m)
        
    #         # Term (1): prior on path X
    #         mahal_C_per_d = tf.einsum('dm, dmn, dn -> d', X_cent_T, C_d_invs, X_cent_T)
    #         mahal_C       = tf.reduce_sum(mahal_C_per_d)
    #         logdet_C      = -tf.linalg.logdet(C_d_invs)
    #         m  = tf.cast(tf.shape(X)[0], tf.float64)
    #         term1 = tf.reduce_sum(m * tf.math.log(tf.constant(2.0 * np.pi, dtype=tf.float64)) + logdet_C) + mahal_C
        
    #         # Term (3): ODE mismatch
    #         f_vals_T      = tf.transpose(f_vec(I, X, thetas))      # (D, m)
    #         m_times_xcent = tf.einsum('dmn, dn -> dm', m_ds, X_cent_T)
    #         resid_T       = f_vals_T - m_times_xcent
    #         mahal_K_per_d = tf.einsum('dm, dmn, dn -> d', resid_T, K_d_invs, resid_T)
    #         mahal_K       = tf.reduce_sum(mahal_K_per_d)
    #         logdet_K      = -tf.linalg.logdet(K_d_invs)
    #         term3 = tf.reduce_sum(m * tf.math.log(tf.constant(2.0 * np.pi, dtype=tf.float64)) + logdet_K) + mahal_K
        
    #         # Term (2): observation noise and error
    #         log_det_Sigma = tf.reduce_sum(N_ds * tf.math.log(tf.constant(2.0 * np.pi, dtype=tf.float64) * sigma_sqs))
    #         X_flat        = tf.reshape(X, [-1])
    #         X_obs         = tf.gather(X_flat, not_nan_idxs)
    #         mahal_Sigma   = tf.reduce_sum(tf.square(X_obs - y_tau_ds_observed) * tf.gather(1.0 / sigma_sqs, not_nan_cols))
    #         term2 = log_det_Sigma + mahal_Sigma
        
    #         base = -0.5 * ((1.0 / beta) * (term1 + term3) + term2)  # same tempering structure as before
    #         return beta_temp * (base + log_jac_sigma + log_jac_theta)

    #     # --- softplus inverse helper for MAP init ---
    #     def inv_softplus(y: tf.Tensor) -> tf.Tensor:
    #         # y > 0; return z s.t. softplus(z) ≈ y
    #         eps = tf.constant(1e-8, tf.float64)
    #         return tf.math.log(tf.maximum(tf.exp(y) - 1.0, eps))
        
        # # initial states in pre-softplus space (as in your sampler)
        # sigma_pre_init = np.full_like(self.sigma_sqs_init, -5.0, dtype=np.float64)
        # pos = self.sigma_sqs_init > sigma_sqs_LB
        # sigma_pre_init[pos] = np.log(np.exp((self.sigma_sqs_init - sigma_sqs_LB)[pos]) - 1.0)
        
        # theta_pre_init = np.full_like(self.thetas_init, -5.0, dtype=np.float64)
        # pos_t = self.thetas_init > 0.0
        # theta_pre_init[pos_t] = np.log(np.exp(self.thetas_init[pos_t]) - 1.0)
        
        # # Shared current state tensors
        # X_state      = tf.Variable(self.Xhat_init, dtype=tf.float64, name="X_state")
        # sigma_pre    = tf.Variable(sigma_pre_init, dtype=tf.float64, name="sigma_pre")
        # theta_pre    = tf.Variable(theta_pre_init, dtype=tf.float64, name="theta_pre")
        
        # # =========================
        # # MAP branch (no sampling)
        # # =========================
        # if method.lower() == "map":
        #     opt_X     = tf.keras.optimizers.Adam(map_lr_X)
        #     opt_sigma = tf.keras.optimizers.Adam(map_lr_sigma)
        #     opt_theta = tf.keras.optimizers.Adam(map_lr_theta)
        
        #     @tf.function
        #     def step():
        #         with tf.GradientTape(persistent=True) as tape:
        #             nlp = -sampler_log_prob(X_state, sigma_pre, theta_pre, beta_temp=tf.constant(map_beta_temp, tf.float64))
        #         gX  = tape.gradient(nlp, X_state)
        #         gs  = tape.gradient(nlp, sigma_pre)
        #         gt  = tape.gradient(nlp, theta_pre)
        #         opt_X.apply_gradients([(gX, X_state)])
        #         opt_sigma.apply_gradients([(gs, sigma_pre)])
        #         opt_theta.apply_gradients([(gt, theta_pre)])
        #         return nlp
        
        #     best_nlp = np.inf
        #     stall = 0
        #     best_pack = (self.Xhat_init.copy(),
        #                  (tf.math.softplus(sigma_pre).numpy() + sigma_sqs_LB),
        #                  tf.math.softplus(theta_pre).numpy())
        ...
        #         "I": self.I,
        #         "X_map": X_map,
        #         "sigma_sqs_map": sigma_map,
        #         "thetas_map": theta_map,
        #         "logpost": -best_nlp,
        #         "minutes_elapsed": 0.0,
        #         "method": "map"
        #     }
        ...
        # =========================
        # MCMC branch (original)
        # =========================
        # Build kernels using sampler_log_prob as the single source of truth.
        # def target_log_prob_fn(X, sigma_sqs_pre, thetas_pre):
        #     return sampler_log_prob(X, sigma_sqs_pre, thetas_pre, beta_temp=tf.constant(1.0, tf.float64))
        
        # beta_temp_init = logarithmic_temperature_schedule(step=0, min_temp=0.1)
        
        # adaptive_sampler = tfp.mcmc.DualAveragingStepSizeAdaptation(
        #     inner_kernel=tfp.mcmc.NoUTurnSampler(
        #         target_log_prob_fn=(lambda X, s_pre, th_pre:
        #                             sampler_log_prob(X, s_pre, th_pre, beta_temp=beta_temp_init)),
        #         step_size=0.1),
        #     num_adaptation_steps=int(0.8 * num_burnin_steps),
        #     target_accept_prob=0.75
        # )
        ...
        # final_sampler = adaptive_sampler
        # if tempering:
        #     final_sampler = LogAnnealedNUTS(
        #         base_kernel=adaptive_sampler,
        #         num_steps=num_burnin_steps + num_results,
        #         unnormalized_log_prob=lambda X, s_pre, th_pre, beta_temp: sampler_log_prob(X, s_pre, th_pre, beta_temp),
        #         min_temp=0.1
        #     )
        
        # # initial state
        # initial_state = [self.Xhat_init, sigma_pre_init, theta_pre_init]
        ...
        # if verbose:
        #     print(f"Finished sampling in {minutes:.2f} minutes.")
        
        # # package results (same shapes/names as your original)
        # return {
        #     "phi1s": self.phi1s, "phi2s": self.phi2s,
        #     "Xhat_init": self.Xhat_init,
        #     "sigma_sqs_LB": sigma_sqs_LB,
        #     "sigma_sqs_init": self.sigma_sqs_init,
        #     "sigma_sqs_init_constrained": tf.math.softplus(sigma_pre_init) + sigma_sqs_LB,
        #     "thetas_init": self.thetas_init,
        #     "thetas_init_constrained": tf.math.softplus(theta_pre_init).numpy(),
        #     "I": self.I,
        #     "X_samps": samples[0].numpy(),
        #     "sigma_sqs_samps": tf.math.softplus(samples[1]).numpy() + sigma_sqs_LB,
        #     "thetas_samps": tf.math.softplus(samples[2]).numpy(),
        #     "kernel_results": kernel_results,
        #     "sample_results": samples,
        #     "minutes_elapsed": minutes,
        #     "method": "mcmc"
    ...
    def update_kernel_matrices(self, I_new : np.ndarray, 
                               phi1s_new : np.ndarray, 
                               phi2s_new : np.ndarray) -> None:
        '''Updates kernel matrices given new discretization points and Matern kernel hyperparameters.
        
        Args:
            I_new (np.ndarray): new set of discretization points for evaluation.
            phi1s_new (np.ndarray): new set of phi1 Matern kernel hyperparameters (fixed).
            phi1s_new (np.ndarray): new set of phi2 Matern kernel hyperparameters (fixed).
        '''
    ...
    '''
    # adds in 2^discretization - 1 evenly-spaced timesteps between consecutive observations
    def _discretize(self, ts_obs : np.ndarray, 
                    X_obs : np.ndarray, 
                    discretization : int) -> np.ndarray:
        
        '''Adds in 2^discretization - 1 evenly-spaced timesteps between consecutive observations
        
        Args:
            ts_obs (np.ndarray): timesteps of raw observations.
            X_obs (np.ndarray): (presumably noisy) observations. Each row contains one timestep's components.
            discretization (int): 2^discretization - 1 points are inserted between raw observation timesteps.

        Returns:
            I (np.ndarray): discretized timesteps.
            X_obs_discret (np.ndarray): matrix with augmented NaN-containing rows.
        '''
    ...
    def _linear_interpolate(self, X_partial : np.ndarray) -> np.ndarray:
        
        '''Takes in data matrix X and linearly-interpolates the NaNs.

        Args:
            X_partial (np.ndarray): data matrix containing rows with potential NaNs.

        Returns:
            X_interp (np.ndarray): data matrix with same shape as X_partial with NaN entries linearly interpolated.
    ...
    def _fit_kernel_hparams(self, I : np.ndarray, 
                            X_filled : np.ndarray, 
                            phi1s_exog_filled : Union[np.ndarray, None] = None, 
                            phi2s_exog_filled : Union[np.ndarray, None] = None, 
                            sigma_sqs_exog_filled : Union[np.ndarray, None] = None, 
                            verbose : bool = False) -> dict:
        
        '''Fits Matern kernel hyperparameters given (potentially interpolated) data.
        
        Args:
            I (np.ndarray): discretized timesteps.
            X_filled (np.ndarray): data matrix assuming each column has no NaNs (i.e., actually observed or interpolated).
            {phi1s_exog_filled, phi2s_exog_filled, sigma_sqs_exog_filled} (np.ndarray/None): user pre-specified values of Matern kernel hyperparameter values. Can fit only unspecified entries.
            verbose (bool): progress bars or no?

        Returns:
            dict containing fitted and/or pre-specified Matern kernel hyperparameters phi1s, phi2s, and sigma_sqs.
        '''
    ...
    def cv_cubic_smoother(self, I : np.ndarray, 
                          X_filled : np.ndarray) -> np.ndarray:

        '''Cubic-spline smoothing to encourage smoother Gaussian process kernel initial hyperparameter fits, especially with missing components.
        
        Args:
            I (np.ndarray): discretization timepoints.
            X_filled (np.ndarray): data matrix with no NaNs (e.g., after linear interpolation).

        Returns:
            np.ndarray: cubic-spline smoothed initial values, with knots chosen via cross-validation.
        '''
    ...
    def single_cv_cubic_smoother(self, I : np.ndarray,
                                 x : np.ndarray) -> np.ndarray:
        
        '''Uses 5-fold cross-validation to select number of knots governing spline expressivity and smooth our initial values for one component.

        Args:
            I (np.ndarray): discretized timesteps.
            x (np.ndarray): values of a single component with timesteps corresponding to I. Assumed to have no NaNs.

        Returns:
            x_smoothed (np.ndarray): smoothed timeseries for specified component.
        '''
    ...
    def _build_matrices(self, I : np.ndarray, 
                        phi1 : float, 
                        phi2 : float, 
                        v : float =2.01) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        '''Takes in discretized timesteps I and hparams (phi1, phi2, v). Returns (C_d, m_d, K_d) for component d.
        
        Args:
            I (np.ndarray) of discretized timesteps, phi1 & phi2 are floats.
            {phi1, phi2} (float): Matern kernel hyperparameters.
            v (int): Matern kernel degrees of freedom.

        Returns:
            C_d, m_d, K_d (np.ndarray): kernel matrices for dth component.
        '''
...
def logarithmic_temperature_schedule(step : Union[int, tf.Tensor], min_temp : float = 0.1):
    ...
    Args:
        step (int/tf.Tensor): which step of sampling are we on?
        min_temp (float): what is the minimum temperature?
    
    Returns:
        beta_temp (float/tf.Tensor): the multiplier we will place on our log-posterior for temperature control.
    '''
