...
def parse_args(argv):
    ap = argparse.ArgumentParser(description="Change-point detection runner for SEIRD (beta or pd change).")
    ...
    ap.add_argument("--prefix", type=str, default="seird_beta_run", help="prefix for output files.")
    ...
    ap.add_argument("--model", type=str, default="", help="Model override; otherwise decided by --mode.")
    ap.add_argument("--mode", type=str, choices=["beta", "pd"], default="beta", help="Which SEIRD change type to simulate/detect.")
...
def resolve_model(args):
    if args.model:
        return args.model
    return "seird_pd" if args.mode == "pd" else "seird_beta"
...
if __name__ == "__main__":
    ...
    if args.prefix == "seird_beta_run" and args.mode == "pd":
        args.prefix = "seird_pd_run"
    if args.child:
        child_main(args)
    else:
        parent_main(args)
